require "application_system_test_case"

class OutputsTest < ApplicationSystemTestCase
  setup do
    @output = outputs(:one)
  end

  test "visiting the index" do
    visit outputs_url
    assert_selector "h1", text: "Outputs"
  end

  test "creating a Output" do
    visit outputs_url
    click_on "New Output"

    fill_in "Date", with: @output.date
    fill_in "Float", with: @output.float
    fill_in "Int", with: @output.int
    fill_in "String", with: @output.string
    fill_in "Text", with: @output.text
    fill_in "Tinyint", with: @output.tinyint
    click_on "Create Output"

    assert_text "Output was successfully created"
    click_on "Back"
  end

  test "updating a Output" do
    visit outputs_url
    click_on "Edit", match: :first

    fill_in "Date", with: @output.date
    fill_in "Float", with: @output.float
    fill_in "Int", with: @output.int
    fill_in "String", with: @output.string
    fill_in "Text", with: @output.text
    fill_in "Tinyint", with: @output.tinyint
    click_on "Update Output"

    assert_text "Output was successfully updated"
    click_on "Back"
  end

  test "destroying a Output" do
    visit outputs_url
    page.accept_confirm do
      click_on "Destroy", match: :first
    end

    assert_text "Output was successfully destroyed"
  end
end
